#ifndef GAZEBO_ROS_NEATO_H
#define GAZEBO_ROS_NEATO_H
...
class GazeboRosNeato : public ModelPlugin
{
public:
  /// Constructor
  GazeboRosNeato();
...
//  sensors::RaySensorPtr cliff_sensor_center_;
  ...
  /// Neato ROS message for cliff event
  ...
  int floot_dist_;
  /// Pointer to bumper sensor simulating Neato's left, centre and right bumper sensors
  sensors::ContactSensorPtr bumper_;
  /// ROS publisher for bumper events
  ros::Publisher bumper_event_pub_;
  /// Neato ROS message for bumper event
