...
void GazeboRosNeato::updateJointState()
{
  /*
   * Joint states
   */
 ...
 */
void GazeboRosNeato::updateOdometry(common::Time& step_time)
{
  ...
  if (isnan(d1))
  {
    ROS_WARN_STREAM_THROTTLE(0.1, "Gazebo ROS Neato plugin: NaN in d1. Step time: " << step_time.Double()
                             << ", WD: " << wheel_diam_ << ", velocity: " << joints_[LEFT]->GetVelocity(0));
    d1 = 0;
  }
  if (isnan(d2))
  {
    ROS_WARN_STREAM_THROTTLE(0.1, "Gazebo ROS Neato plugin: NaN in d2. Step time: " << step_time.Double()
  ...
  // Just as in the Neato driver, the angular velocity is taken directly from the IMU
 ...
 */
void GazeboRosNeato::propagateVelocityCommands()
{
 ...
 */
void GazeboRosNeato::updateCliffSensor()
{
  // Left cliff sensor
...
// +30 ... -30: centre bumper
// -30 ... -90: right bumper
void GazeboRosNeato::updateBumper()
{
  // reset flags
