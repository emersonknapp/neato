 ...
 */
void GazeboRosNeato::prepareMotorPower() {
 ...
 */
bool GazeboRosNeato::prepareJointState()
{
 ...
 */
void GazeboRosNeato::preparePublishTf()
{
...
bool GazeboRosNeato::prepareWheelAndTorque()
{
...
void GazeboRosNeato::prepareOdom()
{
  odom_pose_[0] = 0.0;
  odom_pose_[1] = 0.0;
  odom_pose_[2] = 0.0;
 ...
 */
bool GazeboRosNeato::prepareVelocityCommand()
{
...
bool GazeboRosNeato::prepareCliffSensor()
{
  /*
 ...
 * Prepare bumper
 */
bool GazeboRosNeato::prepareBumper()
{
 ...
 * Prepare IMU
 */
bool GazeboRosNeato::prepareIMU()
{
  std::string imu_name;
...
void GazeboRosNeato::setupRosApi(std::string& model_name)
{
  ...
  motor_power_sub_ = gazebo_ros_->node()->subscribe(motor_power_topic, 10, &GazeboRosNeato::motorPowerCB, this);
  ...
  odom_reset_sub_ = gazebo_ros_->node()->subscribe(odom_reset_topic, 10, &GazeboRosNeato::resetOdomCB, this);
  ROS_INFO("%s: Try to subscribe to %s!", gazebo_ros_->info(), odom_reset_topic.c_str());

  // cmd_vel
  std::string cmd_vel = base_prefix + "/commands/velocity";
  cmd_vel_sub_ = gazebo_ros_->node()->subscribe(cmd_vel, 100, &GazeboRosNeato::cmdVelCB, this);
  ROS_INFO("%s: Try to subscribe to %s!", gazebo_ros_->info(), motor_power_topic.c_str());
