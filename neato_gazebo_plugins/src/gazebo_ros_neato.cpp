...
#include "gazebo_ros_neato.h"

namespace gazebo
{


GazeboRosNeato::GazeboRosNeato() : shutdown_requested_(false)
{
...
void GazeboRosNeato::Load(physics::ModelPtr parent, sdf::ElementPtr sdf)
{
  model_ = parent;
  if (!model_)
  {
  ...
  gazebo_ros_ = GazeboRosPtr(new GazeboRos(model_, sdf, "Neato"));
  sdf_ = sdf;
  ...
  ROS_INFO_STREAM("GazeboRosNeato plugin ready to go! [" << node_name_ << "]");
  update_connection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&GazeboRosNeato::OnUpdate, this));
}


void GazeboRosNeato::OnUpdate()
{
  /*
...
void GazeboRosNeato::motorPowerCB(const kobuki_msgs::MotorPowerPtr &msg)
{
...
void GazeboRosNeato::cmdVelCB(const geometry_msgs::TwistConstPtr &msg)
{
...
void GazeboRosNeato::resetOdomCB(const std_msgs::EmptyConstPtr &msg)
{
  odom_pose_[0] = 0.0;
  odom_pose_[1] = 0.0;
  odom_pose_[2] = 0.0;
...
GZ_REGISTER_MODEL_PLUGIN(GazeboRosNeato);
